...
class Slug
{
	 ...
	 * @return  string
	 */
	public function create_uri($data = '', $id = '', $db = '')
	{
		if(empty($data))
		{
			return FALSE;
		...
		if(is_array($data))
		{
			...
			{
				return $this->_check_uri($this->create_slug($data[$this->field]), $id, '', $db);
			}
			else if(!empty($data[$this->title]))
			{
				return $this->_check_uri($this->create_slug($data[$this->title]), $id, '', $db);
			}
		}
		else if(is_string($data))
		{
			return $this->_check_uri($this->create_slug($data), $id, '', $db);
		}
	 ...
	 * @return  string
	 */
	private function _check_uri($uri, $id = FALSE, $count = 0, $db='')
	{
		$CI 	 =& get_instance();
		$newdb   = $CI->db;
		...
		// Setup the query
		$newdb->select($this->field)->where($this->field, $new_uri);

		if($id)
		{
			$newdb->where($this->id.' !=', $id);
		}

		if($newdb->count_all_results($this->table) > 0)
		{
			return $this->_check_uri($uri, $id, ++$count, $db);
		}
		else
		{
			return $new_uri;
