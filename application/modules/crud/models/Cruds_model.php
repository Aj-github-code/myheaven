    ...
    class Cruds_model extends CI_Model
    {
        ...
        function get_data($conditions=[], $limit='', $offset=0, $allcount='')
        {
            ...
            if(isset($_GET['search']) && !empty($_GET['search']) && $_GET['search'] != 'undefined')
            {
                ...
                $like_sql   = ' AND
                                (
                                    name LIKE "%'.$term.'%" ESCAPE "!"
                                    OR email LIKE "%'.$term.'%" ESCAPE "!"
                                    OR mobile LIKE "%'.$term.'%" ESCAPE "!"
                                    OR pincode LIKE "%'.$term.'%" ESCAPE "!"
                                    OR created_on LIKE "%'.$term.'%" ESCAPE "!"
                                )
                            ';
            }
            ...
            if(isset($_GET['custom_search']) && !empty($_GET['custom_search']))
            {
                ...
                {
                    $where      .= ' AND created_on BETWEEN "'.$from_date.'" AND "'.$to_date.'"';
                }
                else if(!empty($from_date) && empty($to_date))
                {
                    $where      .= ' AND created_on >= "'.$from_date.'"';
                }
                else if(empty($from_date) && !empty($to_date))
                {
                    $where      .= ' AND created_on <= "'.$to_date.'"';
                }
