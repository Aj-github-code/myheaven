<?php
    ...
    class Customer_orders extends MY_Controller {
        ...
        function index()
        {
            $data['meta_title']             = "Customer Orders";
            $data['meta_description']       = "Customer Orders";
            $data['meta_keywords']          = "Customer Orders";
            $data['page_title']             = "Customer Orders";
            $data['module']                 = "Customer Orders";
            $data['menu']                   = "customer_orders";
            $data['submenu']                = "list";
            $data['childmenu']              = "";
            $data['loggedin']               = "yes";

            $this->breadcrumbs->push(1, 'Customer Orders', customer_orders_constants::customer_orders_url);
        ...
        function add()
        {
            ...
            $this->breadcrumbs->unshift(1, 'Customer Orders', customer_orders_constants::customer_orders_url);
            ...
            if($this->form_validation->run($this) === TRUE)
            {
                if((isset($_POST['orderproductid']) && !empty($_POST['orderproductid'])) && (isset($_POST['customer_id']) && !empty($_POST['customer_id'])))
                {
                    ...
                    foreach ($order_products as $key => $value) {
                        ...
                        $amount             = $mrp;
                        $total_amount       = $mrp*$quantity;

                        $d_p_amount         = $d_p;
                        $total_d_p          = $d_p*$quantity;

                        $b_v_amount         = $b_v;
                        $total_b_v          = $b_v*$quantity;
                        ...
                        else
                        {
                            $order_request[]= [
                                                ...
                                                'order_d_p_amount'      => $d_p_amount,
                                                'order_total_d_p_amount'=> $total_d_p,
                                                'order_b_v_amount'      => $b_v_amount,
                        ...
                        $final_d_p          = $final_d_p+$total_d_p;
                        $final_b_v          = $final_b_v+$total_b_v;
                    }
                    ...
                    $order_number                   = 'C-ORDER-'.strtoupper($this->common_lib->generate_random_string(6));
                    $save_order                     = [];
                    ...
                    $save_order['total_amount']     = $final_amount;
                    $save_order['total_d_p']        = $final_d_p;
                    $save_order['total_b_v']        = $final_b_v;
                    $save_order['total_quantity']   = $final_quantity;
