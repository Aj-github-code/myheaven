<?php
    ...
    class Messages_model extends CI_Model
    {
        ...
        function save($franchise_id, $id, $post_data=[])
        {
            ...
            if(!empty($post_data))
            {
                ...
                if(!empty($id))
                {
                    ...
                    $this->db->where('type', 'franchise');
                    $this->db->update($this->table_news, $save);
                }
                else
                {
                    $save['type']                       = 'franchise';
                    ...
                    $this->db->insert($this->table_news, $save);
                }

                if($this->db->affected_rows())
                {
                    $msg = ['error' => 0, 'message' => 'Message successfully saved'];
                }
                else
                {
                    $msg = ['error' => 1, 'message' => 'Unable to save message'];
                }
            }
            return $msg;
        }
        ...
        function change($id='', $status)
        {
            ...
            $this->db->where('id', $id);
            $this->db->where('type', 'franchise');
            $this->db->update($this->table_news, $update);
