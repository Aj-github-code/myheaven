<?php
    ...
    class Top_up extends MY_Controller {
        ...
        function __construct() {
            ...
            $this->load->model('top_up_model', 'top_up');
            $this->load->model('commissions/commissions_model', 'commissions');
        }

        function index()
        {
            $data['meta_title']             = "Top Up";
            $data['meta_description']       = "Top Up";
            $data['meta_keywords']          = "Top Up";
            ...
            $this->breadcrumbs->push(1, 'Top Up', top_up_constants::top_up_url);
            $this->breadcrumbs->unshift(2, 'List', '#');
            $data['breadcrumb']             = $this->breadcrumbs->show();

            $data['content']                = "top_up/index";
            echo Modules::run("templates/".$this->config->item('theme'), $data);
        }

        function ajax_list()
        {
            $condition              = ['tu.status !' => '-1', 'tu.created_by' => $this->session->userdata('user_id')];
            $list                   = $this->top_up->get_data($condition, '', '', '');
            $tabledata              = [];
            ...
            foreach ($list as $key => $value) {
                ...
                $order_delete       = '
                                        <a class="text-danger m-l-10" title="Delete" href="javascript:void(0);" onclick="change_status(`'.$value->order_number.'`, -1, this);" data-type="top up" data-function="'.base_url().top_up_constants::change_top_up_status_url.'"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                    ';
                $order_status       = '';
                $order_print        = '
                                        <a class="text-warning m-l-10" title="Print" href="'.base_url().top_up_constants::print_top_up_url.'/'.$value->order_number.'" target="_blank"><i class="fa fa-print" aria-hidden="true"></i></a>
                                    ';
                ...
                $action             = '
                                        <span class="d-flex justify-content-center">
                                            <a class="" title="View" href="'.base_url().top_up_constants::view_top_up_url.'/'.$value->order_number.'"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                            '.$order_print.'
                                            '.$order_delete.'
                                        </span>
                                    ';

                $view               = '<a class="" href="'.base_url().top_up_constants::view_top_up_url.'/'.$value->order_number.'" title="View '.$value->order_number.'">'.$value->order_number.'</a>';
            ...
            $output                 = array(
                                        "total"      => $this->top_up->get_data($condition, '', '', 'allcount'),
                                        "rows"       => $tabledata,
                                    );
        ...
        function add()
        {
            $data['meta_title']             = "Add Top Up";
            $data['meta_description']       = "Add Top Up";
            $data['meta_keywords']          = "Add Top Up";
            $data['page_title']             = "Add Top Up";
            $data['module']                 = "Top Up";
            $data['menu']                   = "top_up";
            $data['submenu']                = "add";
            $data['childmenu']              = "";
            ...
            $this->breadcrumbs->unshift(1, 'Top Up', top_up_constants::top_up_url);
            $this->breadcrumbs->unshift(2, 'Add', '#');
            $data['breadcrumb']             = $this->breadcrumbs->show();

            $data['products']               = $this->top_up->get_my_products();
            $data['content']                = "top_up/form";
            echo Modules::run("templates/".$this->config->item('theme'), $data);
        }
        ...
        function place()
        {
            ...
            if($this->form_validation->run($this) === TRUE)
            {
                if((isset($_POST['orderproductid']) && !empty($_POST['orderproductid'])) && (isset($_POST['member_code']) && !empty($_POST['member_code'])))
                {
                    ...
                    $member                 = $this->top_up->get_member($member_code);
                    $response               = $this->validate_member($member, $package);

                    if($response['error'] == 1)
                    {
                        $this->session->set_flashdata('status', ['error' => 1, 'message' => $response['message']]);
                        redirect(base_url().top_up_constants::add_top_up_url);
                    }
                    ...
                    {
                        $gst_type           = $this->top_up->get_gst_type($oneproductid);
                    }
                    ...
                    foreach ($order_products as $key => $value) {
                        ...
                        $product            = $this->top_up->get_product($product_id);
                    ...
                    $response                       = $this->top_up->place($save_order, $order_request);

                    $redirect                       = base_url(top_up_constants::add_top_up_url);
                    if($response['error'] == 0)
                    {
                        $this->commissions->franchise_and_sponsor('top_up', $response['order_id'], $this->data['user_data']);
                        $redirect                   = base_url(top_up_constants::view_top_up_url.'/'.$response['order_number']);
                    }
        ...
        function view($order_number)
        {
            ...
            {
                $data['meta_title']         = "View Top Up";
                $data['meta_description']   = "View Top Up";
                $data['meta_keywords']      = "View Top Up";
                $data['page_title']         = "View Top Up";
                $data['module']             = "Top Up";
                $data['menu']               = "top_up";
                $data['submenu']            = "list";
                $data['childmenu']          = "";
                ...
                $data['order']              = $this->top_up->get_details($order_number);
                
                if(!empty($data['order']))
                {
                    $order_id               = $data['order']['id'];
                    $data['id']             = $order_id;
                    $data['order_products'] = $this->top_up->get_order_products($order_id);
                    ...
                    $data['content']        = "top_up/view";
                    echo Modules::run("templates/".$this->config->item('theme'), $data);
                }
                else
                {
                    $this->session->set_flashdata('status', ['error' => 1, 'message' => 'Top up not found']);
        ...
        function print_pdf($order_number)
        {
            ...
            if(!empty($order_number))
            {
                $data['meta_title']         = "Print Top Up";
                $data['meta_description']   = "Print Top Up";
                $data['meta_keywords']      = "Print Top Up";
                $data['page_title']         = "Print Top Up";
                $data['module']             = "Top Up";
                $data['menu']               = "top_up";
                $data['submenu']            = "list";
                $data['childmenu']          = "";
                ...
                if(!empty($data['order']))
                {
                    $order_id               = $data['order']['id'];
                    $data['id']             = $order_id;
                    $data['member_details'] = $this->top_up->get_member($data['order']['member_code']);
                    $data['order_products'] = $this->top_up->get_order_products($order_id);
                    // echo "<pre>";print_r($data);exit;
                    ...
                    $pdf_data               = [
                                                ...
                                                'html'      => $this->load->view("top_up/pdf/invoice", $data, true),
        ...
        function process()
        {
            ...
            if(isset($_POST['order_number']) && !empty($_POST['order_number']))
            {
                ...
                $order                      = $this->top_up->get_details($order_number);

                if(!empty($order))
                {
                    $response               = $this->top_up->process($order, $_POST);
                    $this->session->set_flashdata('status', $response);
                    redirect(base_url().top_up_constants::view_top_up_url.'/'.$order_number);
                }
                else
                {
                    $this->session->set_flashdata('status', ['error' => 1, 'message' => 'Top up not found']);
        ...
        function change()
        {
            ...
            if(isset($_POST) && !empty($_POST))
            {
                ...
                $details        = $this->top_up->get_details($order_number);
                ...
                if(!empty($details))
                {
                    ...
                    if($this->top_up->change($details, $status))
                    {
        ...
        function validate_member($member, $package)
        {
            if(!empty($member))
            {
                $pin                = $member['pin'];
                ...
                {
                    $response       = ['error' => 1, 'message' => 'You can not downgrade package'];
                }
                else if($package == $pin)
                {
                    $response       = ['error' => 1, 'message' => 'This member has same package'];
                }
            }
            else
            {
        ...
        function check_member()
        {
            ...
            if((isset($_POST['member_code']) && !empty($_POST['member_code'])) && (isset($_POST['package']) && !empty($_POST['package'])))
            {
                $member_code        = $_POST['member_code'];
                $package            = $_POST['package'];
                $member             = $this->top_up->get_member($member_code);
