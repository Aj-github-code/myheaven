    ...
    class Top_up_model extends CI_Model
    {
        ...
        function get_data($conditions=[], $limit='', $offset=0, $allcount='')
        {
            ...
            if(isset($_GET['search']) && !empty($_GET['search']) && $_GET['search'] != 'undefined')
            {
                ...
                $like_sql   = ' AND
                                (
                                    tu.order_number LIKE "%'.$term.'%" ESCAPE "!"
                                    OR tu.package LIKE "%'.$term.'%" ESCAPE "!"
                                    OR tu.total_d_p LIKE "%'.$term.'%" ESCAPE "!"
                                    OR tu.total_quantity LIKE "%'.$term.'%" ESCAPE "!"
                                    OR tu.order_status LIKE "%'.$term.'%" ESCAPE "!"
                                    OR tu.processed_on LIKE "%'.$term.'%" ESCAPE "!"
                                    OR tu.created_on LIKE "%'.$term.'%" ESCAPE "!"
            ...
            if(isset($_GET['custom_search']) && !empty($_GET['custom_search']))
            {
                ...
                {
                    $where      .= ' AND tu.created_on BETWEEN "'.$from_date.'" AND "'.$to_date.'"';
                }
                else if(!empty($from_date) && empty($to_date))
                {
                    $where      .= ' AND tu.created_on >= "'.$from_date.'"';
                }
                else if(empty($from_date) && !empty($to_date))
                {
                    $where      .= ' AND tu.created_on <= "'.$to_date.'"';
                }

                if($order_status != '')
                {
                    $where      .= ' AND tu.order_status IN ("' .implode('", "', $order_status). '")';
                }

                if($status != '')
                {
                    $where      .= ' AND tu.status IN ('.implode(',', $status).')';
                }
            }
            ...
            if(isset($_GET['order']) && !empty($_GET['order']) && isset($_GET['sort']) && !empty($_GET['sort']))
            {
                ...
                {
                    $sort_by = 'tu.created_on';
                }
                else if($_GET['sort'] == 'order_number')
                {
                    $sort_by = 'tu.order_number';
                }
                else if($_GET['sort'] == 'package')
                {
                    $sort_by = 'tu.package';
                }
                else if($_GET['sort'] == 'total_d_p')
                {
                    $sort_by = 'tu.total_d_p';
                }
                else if($_GET['sort'] == 'total_quantity')
                {
                    $sort_by = 'tu.total_quantity';
                }
            ...
            $this->db->select('tu.*, u.firstname, u.ownid');
            $this->db->from($this->table_top_up_orders.' tu');
            $this->db->join($this->myheavendb->database.'.'.$this->table_userdetails.' u', 'tu.member_code=u.ownid', 'INNER');
            $this->db->where($where);
        ...
        {
            $this->db->select('*');
            $this->db->from($this->table_top_up_orders);
        ...
        {
            $this->db->select('tuop.*, p.name, p.p_code, p.hsn_sac, p.slug, p.description, p.thumbnail, p.mrp, p.d_p, p.b_v, p.opening_stock');
            $this->db->from($this->table_top_up_order_products.' tuop');
            $this->db->join($this->table_my_products.' p', 'tuop.product_id=p.product_id', 'INNER');
            $this->db->where('tuop.order_id', $order_id);
            $this->db->where('tuop.user_id', $this->session->userdata('user_id'));
            $this->db->where('tuop.status', 1);
            $this->db->where('p.user_id', $this->session->userdata('user_id'));
            $this->db->where('p.status', 1);
            $this->db->order_by('tuop.id', 'ASC');
            $query = $this->db->get();
            return $query->result_array();
        }
        ...
        function place($save_order, $order_request)
        {
            ...
            $this->db->insert($this->table_top_up_orders, $save_order);
            if($this->db->affected_rows())
            {
                ...
                foreach ($order_request as $key => $value) {
                    ...
                    $this->db->insert($this->table_top_up_order_products, $save_order_products);
                    if($this->db->affected_rows())
                    {
               ...
               /* $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL,"https://www.myheavenmarketing.com/admin/api_top_up.php");
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_POSTFIELDS,http_build_query(array("ownid"=>$member_code,"Submit"=>"Submit","pin"=>$save_order['package'])));
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                $server_output = curl_exec($ch);
                curl_close ($ch);*/

                $array_data=array("ownid"=>$member_code,"amount"=>$save_order['package'],"datetime"=>date('Y-m-d H:i:s'),"active"=>0,"type"=>1);
                $this->myheavendb->reset_query();
                $this->myheavendb->insert("franchise_topup_repurchas", $array_data);
                
                $update                 = [];
                // $update['pin']          = $save_order['package'];
                $update['pin_date']     = date('Y-m-d H:i:s');

                $this->myheavendb->reset_query();
                $this->myheavendb->where('ownid', $member_code);
                $this->myheavendb->update($this->table_useroption, $update);
        ...
        function process($order, $post_data)
        {
            ...
            $this->db->where('user_id', $this->session->userdata('user_id'));
            $this->db->update($this->table_top_up_orders, $update);
        ...
        function change($details, $status)
        {
            ...
            if($affected_rows)
            {
                ...
                $this->db->where('user_id', $this->session->userdata('user_id'));
                $this->db->update($this->table_top_up_order_products, $update);
