...
<form id="formId" method="POST" action="<?php echo base_url().top_up_constants::place_top_up_url; ?>">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" id="package" name="package" data-error=".packageerror" style="width: 100%;">
                                    <option value="">-- Select Package --</option>
                                    <option value="500">500 (B. V.)</option>
                                    <option value="1000">1000 (B. V.)</option>
                                    <option value="2000">2000 (B. V.)</option>
                                </select>
                                <div class="packageerror error_msg"></div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
...
<script type="text/javascript">
    ...
    $('#package, #gst_type, #product_id').select2();
    ...
    $("#formId").validate({
        ...
        submitHandler: function(form, event){
            ...
            {
                var package = parse_float($('#package').val());
                // var totalbv = parse_float($('.orderproducttotalbv').val());

                var totalbv = 0;
                $('.orderproducttotalbv').each(function(){
                    totalbv += parse_float($(this).val());
                });
                ...
                if(totalbv >= package)
                {
                    ...
                    $.ajax({
                        type: 'POST',
                        dataType: "json",
                        url: '<?php echo base_url(top_up_constants::check_member_url); ?>',
                        data: {[csrf_name]: csrf_value, member_code: $('#member_code').val(), package: $('#package').val()},
                ...
                {
                    // load_status_popup('error', 'Total bv is '+totalbv+', so package value must be same as bv');
                    load_status_popup('error', 'Total bv should not be less than package bv.');
                }
            }
        }
    });
